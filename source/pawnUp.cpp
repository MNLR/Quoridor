...
bool pawnUp(bool ficha, nodo &in){
 ...
 switch(ficha){
	case true:
		if (in.tablero[in.p1[0]-1][in.p1[1]]==1){  //Comprueba wall obstruyendo movimiento
			return false;
 		}
 		else{
			if (in.tablero[in.p1[0]-2][in.p1[1]]==1){    // Comprueba ficha en posición. 1 si sí.
 				if (in.tablero[in.p1[0]-3][in.p1[1]]==1){   //Comprueba wall tras ficha. 1 si sí.
					return false;
 				}
				else{
		...
		break;
        case false:
                if (in.tablero[in.p2[0]-1][in.p2[1]]==1){  //Comprueba wall obstruyendo movimiento
 			return false;
                }
                else{
                        if (in.tablero[in.p2[0]-2][in.p2[1]]==1){    // Comprueba ficha en posición. 1 si sí.
                                if (in.tablero[in.p2[0]-3][in.p2[1]]==1){   //Comprueba wall tras ficha. 1 si sí.
                                        return false;
                                }
                                else{
